...
export default function AthenaExecutiveBrief() {
  ...
  return (
    <section data-testid="athena-executive-brief" className="rounded-lg border border-[#38BDF8]/35 bg-[#061527] p-5 shadow-[0_28px_90px_rgba(14,165,233,0.10)]">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-[#1D4F73] bg-[#08233D] px-3 text-xs font-semibold text-[#DDF7FF] transition hover:border-[#38BDF8] hover:bg-[#0B2D4D] disabled:opacity-60"
        >
        ...
        <div className="mt-5 space-y-5">
            ...
            <div className="rounded-md border border-[#12314D] bg-[#050F1D] p-4">
              ...
              {brief.decisionOfTheDay ? (
                <div className="mt-3">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#D7E6F8]">{brief.decisionOfTheDay.recommendedAction}</p>
                  <p className="mt-2 text-xs leading-5 text-[#B7C5D8]">Why today: {brief.decisionOfTheDay.whyToday}</p>
                  <p className="mt-2 text-xs leading-5 text-[#B7C5D8]">Expected measurable result: {brief.decisionOfTheDay.expectedMeasurableResult}</p>
                  <p className="mt-2 text-xs text-[#7DD3FC]">Confidence {brief.decisionOfTheDay.confidence}/100</p>
                </div>
              ) : (
                <p className="mt-3 text-sm text-[#B7C5D8]">Insufficient verified data to make a reliable recommendation.</p>
              )}
            </div>
          ...
          <div className="rounded-md border border-[#12314D] bg-[#050F1D] p-4">
            ...
            {brief.memoryInfluence.length > 0 ? (
              <ul className="mt-3 grid gap-2 text-xs leading-5 text-[#B7C5D8] md:grid-cols-2">
                {brief.memoryInfluence.map((item) => <li key={item}>{item}</li>)}
              </ul>
            ) : (
              <p className="mt-3 text-sm text-[#B7C5D8]">No current Business Memory record changed this brief.</p>
            )}
          </div>
        </div>
      )}
...
function AthenaMetric({ icon: Icon, label, value, detail }: { icon: typeof Brain; label: string; value: string; detail: string }) {
  return (
    <div className="rounded-md border border-[#12314D] bg-[#050F1D] p-4">
      ...
      <p className="mt-1 text-xs text-[#7D91AA]">{detail}</p>
    </div>
  );
}
...
function BriefCallout({ label, priority }: { label: string; priority: AthenaPriority | null }) {
  return (
    <div className="mt-3 rounded-md border border-[#12314D] bg-[#061527] p-3">
      ...
      <p className="mt-1 text-xs leading-5 text-[#B7C5D8]">{priority?.observation || "Insufficient verified data to make a reliable recommendation."}</p>
    </div>
  );
}
...
function BriefList({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="rounded-md border border-[#12314D] bg-[#050F1D] p-4">
      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#38BDF8]">{title}</p>
      {items.length > 0 ? (
        <ul className="mt-3 space-y-2 text-xs leading-5 text-[#B7C5D8]">
          {items.slice(0, 5).map((item) => <li key={item}>{item}</li>)}
        </ul>
      ) : (
        <p className="mt-3 text-sm text-[#7D91AA]">Insufficient verified data.</p>
      )}
    </div>
  );
}
