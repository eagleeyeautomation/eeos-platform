...
function LivePreviewPanel({ step, data }: { step: number; data: PRNData }) {
  ...
  return (
    <div className="space-y-4">
      {/* Agency profile */}
      <div className="p-4 rounded-lg bg-[rgba(0,212,200,0.04)] border border-[rgba(0,212,200,0.1)]">
        <div className="text-[10px] text-[#E8EDF5]/35 mb-2"
        ...
        {data.teamSize && (
          <div className="text-xs text-[#E8EDF5]/50 mt-0.5">{data.teamSize}</div>
        )}
      </div>

      {/* Intelligence readiness */}
      <div className="p-4 rounded-lg bg-[rgba(0,212,200,0.04)] border border-[rgba(0,212,200,0.1)]">
        <div className="text-[10px] text-[#E8EDF5]/35 mb-3"
          style={{ fontFamily: "'JetBrains Mono', monospace" }}>INTELLIGENCE READINESS</div>
        {insights.length === 0 ? (
          <div className="text-xs text-[#E8EDF5]/30">Complete the wizard to see your readiness score.</div>
        ) : (
          <div className="space-y-2">
            {insights.map((insight, i) => (
              <div key={i} className="flex items-center gap-2">
                <CheckCircle2 className="w-3 h-3 text-[#00D4C8] shrink-0" />
                <span className="text-xs text-[#E8EDF5]/65">{insight}</span>
              </div>
            ))}
          </div>
        )}
        <div className="mt-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] text-[#E8EDF5]/35"
              style={{ fontFamily: "'JetBrains Mono', monospace" }}>READINESS</span>
            <span className="text-[10px] text-[#00D4C8]"
              style={{ fontFamily: "'JetBrains Mono', monospace" }}>
              {Math.round((step / STEPS.length) * 100)}%
            </span>
          </div>
          <div className="h-1 bg-[rgba(0,212,200,0.08)] rounded-full overflow-hidden">
            <div
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={item.label} className="flex items-center gap-2">
              <item.icon className="w-3 h-3 text-[#00D4C8]/60 shrink-0" />
              <span className="text-xs text-[#E8EDF5]/40">{item.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Security note */}
      <div className="flex items-center gap-2 text-[10px] text-[#E8EDF5]/25"
        style={{ fontFamily: "'JetBrains Mono', monospace" }}>
    ...
    return (
      <div className="min-h-screen bg-[#050C1A] flex items-center justify-center">
        ...
        <div className="max-w-2xl mx-auto px-4 text-center pt-24 pb-16">
          ...
          <h1 className="text-4xl font-bold text-[#E8EDF5] mb-4"
            style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          ...
          <p className="text-lg text-[#E8EDF5]/65 mb-8">
            Your EEOS activation specialist will contact <strong className="text-[#00D4C8]">{data.email}</strong> within 24 hours to begin the GoHighLevel connection and intelligence mapping.
          </p>
        ...
        <div className="flex-1 flex flex-col px-4 sm:px-8 lg:px-16 py-8 sm:py-12 w-full lg:max-w-2xl">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[rgba(0,212,200,0.08)] border border-[rgba(0,212,200,0.2)]">
                <div className="w-1.5 h-1.5 rounded-full bg-[#00D4C8]" />
                <span className="text-[10px] text-[#00D4C8]"
                  style={{ fontFamily: "'JetBrains Mono', monospace" }}>STAFFING EDITION</span>
              </div>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#E8EDF5] tracking-tight leading-tight"
              style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
              {STEPS[step - 1].title}
            </h1>
              ...
              <div key={s.id} className="flex items-center">
                ...
                {i < STEPS.length - 1 && (
                  <div className="h-px w-4 transition-all duration-500"
                    style={{ background: step > s.id ? "rgba(0,212,200,0.4)" : "rgba(0,212,200,0.08)" }} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="flex-1">
            ...
            {step === 2 && (
              <div className="space-y-5">
                <p className="text-sm text-[#E8EDF5]/55">Select all staffing markets PRN Staffers operates in.</p>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2.5 px-4 py-3 rounded-lg text-sm text-left transition-all duration-200 ${
                        data.markets.includes(market)
                          ...
                          : "bg-[rgba(15,30,53,0.6)] border border-[rgba(0,212,200,0.1)] text-[#E8EDF5]/65 hover:border-[rgba(0,212,200,0.25)]"
                      }`}
                      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                    >
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-3">
                    ...
                    <button
                      type="button"
                      onClick={addLocation}
                      className="px-4 py-2.5 text-sm font-semibold text-[#050C1A] bg-[#00D4C8] rounded-lg hover:bg-[#00E8DB] transition-all duration-200"
                      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                    >
                      Add
                    </button>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {data.officeLocations.map((loc) => (
                      <span key={loc} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-[rgba(0,212,200,0.1)] border border-[rgba(0,212,200,0.25)] text-[#00D4C8]"
                        style={{ fontFamily: "'JetBrains Mono', monospace" }}>
            ...
            {step === 4 && (
              <div className="space-y-4">
                <p className="text-sm text-[#E8EDF5]/55">Select the intelligence objectives most important to PRN Staffers leadership.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {step === 5 && (
              <div className="space-y-4">
                <p className="text-sm text-[#E8EDF5]/55">Select the KPIs EEOS should monitor and surface in your Executive Dashboard.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {step === 6 && (
              <div className="space-y-5">
                <p className="text-sm text-[#E8EDF5]/55">Select the software systems PRN Staffers currently uses. EEOS will map integration connectors for each.</p>
                {SOFTWARE_OPTIONS.map((cat) => (
                  <div key={cat.category}>
                    <div className="text-xs text-[#E8EDF5]/45 mb-2 uppercase tracking-wider"
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs transition-all duration-200 ${
                              selected
                                ? "bg-[rgba(0,212,200,0.12)] border border-[rgba(0,212,200,0.4)] text-[#00D4C8]"
                                : "bg-[rgba(15,30,53,0.6)] border border-[rgba(0,212,200,0.1)] text-[#E8EDF5]/60 hover:border-[rgba(0,212,200,0.25)]"
                            }`}
                            style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                          >
                            {opt}
            ...
            {step === 7 && (
              <div className="space-y-6">
                <div className="glass-card rounded-xl p-6 border border-[rgba(0,212,200,0.2)]">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-[#E8EDF5]/45">Authorize read-only access to your GHL account</div>
                    </div>
                  </div>
                  <p className="text-sm text-[#E8EDF5]/60 mb-5 leading-relaxed">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Link
                      href="/connect-ghl"
                      className="flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold text-[#050C1A] bg-[#00D4C8] rounded-lg hover:bg-[#00E8DB] transition-all duration-200 shadow-[0_0_16px_rgba(0,212,200,0.3)]"
                      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                    >
                      <Plug className="w-4 h-4" />
                      Connect GoHighLevel Now
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold text-[#E8EDF5]/55 border border-[rgba(232,237,245,0.1)] rounded-lg hover:border-[rgba(0,212,200,0.2)] transition-all duration-200"
                      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                    >
                ...
                <div className="glass-card rounded-xl p-5">
                  <div className="text-xs text-[#E8EDF5]/35 mb-3"
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <div key={row.label} className="flex items-center justify-between py-1.5 border-b border-[rgba(0,212,200,0.05)]">
                        <span className="text-xs text-[#E8EDF5]/40"
                          style={{ fontFamily: "'JetBrains Mono', monospace" }}>{row.label}</span>
                        <span className="text-xs text-[#E8EDF5]/70"
          ...
          {/* Navigation */}
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-[rgba(0,212,200,0.08)]">
            <button
              type="button"
            ...
            {step < STEPS.length ? (
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-semibold bg-[#00D4C8] text-[#050C1A] hover:bg-[#00E8DB] shadow-[0_0_20px_rgba(0,212,200,0.4)] transition-all duration-200"
                style={{ fontFamily: "'Space Grotesk', sans-serif" }}
              >
                Activate EEOS
                <ArrowRight className="w-4 h-4" />
