                  ...
                  return (
                    <div key={type} className="glass-card rounded-xl p-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-[#E8EDF5]/45 capitalize" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{type.toUpperCase()}S</div>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="glass-card rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between p-4 border-b border-[rgba(255,255,255,0.06)]">
                <div className="flex items-center gap-3">
                  ...
                  {nodes.length > 0 && (
                    <span className="text-xs text-[#E8EDF5]/40" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
              ...
              <div className="relative" style={{ height: "520px", background: "radial-gradient(ellipse at center, rgba(0,212,200,0.03) 0%, transparent 70%)" }}>
                ...
                {!tenantId && !isLoading && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                    <Network className="w-12 h-12 text-[#E8EDF5]/15" />
                    <p className="text-sm text-[#E8EDF5]/40">Connect GoHighLevel to build your Knowledge Graph</p>
                    <Link
                      href="/connect-ghl"
                      className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-[#050C1A] bg-[#00D4C8] rounded-lg hover:bg-[#00E8DB] transition-all duration-200"
                      style={{ fontFamily: "'Space Grotesk', sans-serif" }}
                    >
                      <Zap className="w-4 h-4" />
                      Connect GoHighLevel
                ...
                {tenantId && !isLoading && nodes.length === 0 && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                    <Network className="w-12 h-12 text-[#E8EDF5]/15" />
                    <p className="text-sm text-[#E8EDF5]/40">Knowledge Graph builds as GoHighLevel sends signals to EEOS</p>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {selectedNode.properties && typeof selectedNode.properties === 'object' && 'sublabel' in selectedNode.properties && (
                      <p className="text-sm text-[#E8EDF5]/55">{String(selectedNode.properties.sublabel)}</p>
                    )}
                    <div className="flex flex-wrap gap-4 mt-3 text-xs">
                      <div>
                        <span className="text-[#E8EDF5]/35" style={{ fontFamily: "'JetBrains Mono', monospace" }}>EDGES: </span>
                        <span className="text-[#E8EDF5]/70">
                          {edges.filter((e: LiveEdge) => e.fromNodeId === selectedNode.id || e.toNodeId === selectedNode.id).length}
                        </span>
                      </div>
                      {selectedNode.signalCount != null && (
                        <div>
                          <span className="text-[#E8EDF5]/35" style={{ fontFamily: "'JetBrains Mono', monospace" }}>SIGNALS: </span>
                          <span className="text-[#E8EDF5]/70">{selectedNode.signalCount}</span>
                        </div>
                      )}
                      {selectedNode.externalId && (
                        <div>
                          <span className="text-[#E8EDF5]/35" style={{ fontFamily: "'JetBrains Mono', monospace" }}>GHL ID: </span>
                          <span className="text-[#E8EDF5]/50" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{selectedNode.externalId}</span>
                        </div>
                      )}
                    </div>
                  </div>
