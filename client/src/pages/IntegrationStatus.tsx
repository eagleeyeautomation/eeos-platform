...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="glass-card rounded-lg p-2 text-xs border border-[rgba(0,212,200,0.2)]">
      <div className="text-[#00D4C8]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{label}</div>
      <div className="text-[#E8EDF5]">{payload[0]?.value}% uptime</div>
    </div>
  );
};
          ...
          <AnimatedSection>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <button className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-[#00D4C8] border border-[rgba(0,212,200,0.3)] rounded-lg hover:bg-[rgba(0,212,200,0.08)] active:scale-[0.97] transition-all duration-200" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                  <RefreshCw className="w-4 h-4" />
                  <span className="hidden sm:inline">Refresh All</span>
                </button>
                <Link href="/connect-ghl" className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-[#050C1A] bg-[#00D4C8] rounded-lg hover:bg-[#00E8DB] active:scale-[0.97] transition-all duration-200 shadow-[0_0_14px_rgba(0,212,200,0.3)]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          ...
          <AnimatedSection delay={100}>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: "Records Synced", value: totalRecords.toLocaleString(), color: "#00D4C8", icon: Database },
              ].map((stat) => (
          ...
          <AnimatedSection delay={150}>
            <div className="glass-card rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-base font-bold text-[#E8EDF5]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>System Uptime — Last 24 Hours</h2>
                  <p className="text-xs text-[#E8EDF5]/40 mt-0.5" style={{ fontFamily: "'JetBrains Mono', monospace" }}>Overall: 99.6% uptime</p>
                </div>
      ...
      <section className="py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection delay={200}>
            <h2 className="text-base font-bold text-[#E8EDF5] mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Connector Status</h2>
              ...
              {CONNECTORS.map((connector) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left p-5 rounded-2xl border transition-all duration-200 hover:border-[rgba(0,212,200,0.25)] ${
                      isSelected ? "border-[rgba(0,212,200,0.35)] ring-1 ring-[rgba(0,212,200,0.15)]" : ""
                    }`}
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="text-[10px] text-[#E8EDF5]/40 mb-1" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{connector.category.toUpperCase()}</div>
                        <div className="text-sm font-bold text-[#E8EDF5]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{connector.name}</div>
                      </div>
                    ...
                    <p className="text-xs text-[#E8EDF5]/50 mb-3 leading-relaxed">{connector.description}</p>

                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      <div>
                        <div className="text-[#E8EDF5]/35 mb-0.5" style={{ fontFamily: "'JetBrains Mono', monospace" }}>LAST SYNC</div>
                        <div className="text-[#E8EDF5]/70">{connector.lastSync}</div>
                      </div>
                      <div>
                        <div className="text-[#E8EDF5]/35 mb-0.5" style={{ fontFamily: "'JetBrains Mono', monospace" }}>FREQUENCY</div>
                        <div className="text-[#E8EDF5]/70">{connector.syncFrequency}</div>
                      </div>
                      <div>
                        <div className="text-[#E8EDF5]/35 mb-0.5" style={{ fontFamily: "'JetBrains Mono', monospace" }}>RECORDS</div>
                        <div className="text-[#E8EDF5]/70">{connector.recordsProcessed.toLocaleString()}</div>
                      </div>
                      <div>
                        <div className="text-[#E8EDF5]/35 mb-0.5" style={{ fontFamily: "'JetBrains Mono', monospace" }}>UPTIME</div>
                    ...
                    {isSelected && (
                      <div className="mt-4 pt-3 border-t border-[rgba(255,255,255,0.06)]">
                        <div className="text-[10px] text-[#E8EDF5]/35 mb-1.5" style={{ fontFamily: "'JetBrains Mono', monospace" }}>GHL ENDPOINT</div>
                        <div className="text-[10px] text-[#00D4C8]/70 font-mono">{connector.ghlEndpoint}</div>
        ...
        <section className="py-4 pb-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <AnimatedSection delay={250}>
              <h2 className="text-base font-bold text-[#E8EDF5] mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Recent Errors</h2>
                ...
                {ERROR_LOG.map((err, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-[#E8EDF5]/70">{err.connector}</div>
                      <div className="text-xs text-[#E8EDF5]/50 mt-0.5">{err.error}</div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className="text-[10px] text-[#E8EDF5]/30" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{err.time}</span>
                    </div>
                  </div>
                ))}
              </div>
